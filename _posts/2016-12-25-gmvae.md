...
The improvement is striking! The real GMVAE with proper inference achieves the best performance out of all three models. I ran the same experiment multiple times. By the 1000th epoch, five out of six replicates achieved accuracy performances above 80%. One of them even achieved an accuracy of 95%! Surprisingly, our real GMVAE managed to outperform all of the "GMVAE K=10" models both in terms of best run and average run. Also, in constrast to the models from the paper, I didn't have to rely on convolutional layers, Consistency Violation, or $\eta$, which I consider a personal victory. *\*blows knuckles\**
